...
export default async function Users({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  try {
    ...
    return <><span className="eyebrow">ДОСТУП</span><h1>Users</h1><p>Identity-акаунти Supabase Auth. Це не записи CRM People.</p>
...
function OwnAccount({ email, id }: { email?: string; id: string }) { return <><span className="eyebrow">ДОСТУП</span><h1>Users</h1><section className="card"><h2>Ваш обліковий запис</h2><dl><dt>Email</dt><dd>{email ?? "Не вказано"}</dd><dt>ID</dt><dd>{id}</dd></dl></section><section className="notice"><h2>Потрібна роль адміністратора</h2><p>Лише користувач із наявною роллю <code>app_admin</code> може переглядати інші identity-акаунти та керувати їхніми ролями.</p></section></>; }
function Unavailable() { return <><span className="eyebrow">ДОСТУП</span><h1>Users</h1><section className="notice"><h2>Модель identity ще не застосована</h2><p>Застосуйте міграцію <code>20260914000000_identity_accounts.sql</code>, а потім призначте одному вже наявному Auth-акаунту роль <code>app_admin</code>.</p></section></>; }
